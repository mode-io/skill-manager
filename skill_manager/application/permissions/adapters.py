...
class FileBackedPermissionsAdapter(PermissionHarnessAdapter):
    def __init__(
        self,
        *,
        ...
        self._mapper: PermissionMapper = get_mapper(profile.codec)

    def status(self) -> PermissionHarnessStatus:
        installed = self._is_installed()
        config_present = self.config_path.is_file()
        return PermissionHarnessStatus(
    ...
    def scan(self, specs: tuple[PermissionSpec, ...]) -> PermissionHarnessScan:
        ...
        for raw in raw_entries:
            ...
            if expected == actual and managed_spec.decision == raw.decision and managed_spec.scope == raw.scope:
                entries.append(
                    PermissionObservedEntry(
                        id=raw.id,
                        decision=raw.decision,
                ...
                drift_parts = []
                if managed_spec.decision != raw.decision:
                    drift_parts.append(f"decision: expected={managed_spec.decision}, actual={raw.decision}")
                if managed_spec.scope != raw.scope:
                    drift_parts.append(f"scope: expected={managed_spec.scope}, actual={raw.scope}")
                if expected != actual:
    ...
    def enable_permission(self, spec: PermissionSpec) -> None:
        is_repr, reason, caveat = self._mapper.representable(spec)
        if not is_repr:
            raise MutationError(f"Permission not supported on {self.label}: {reason}", status=400)
    ...
    def _read_entries(self, specs: tuple[PermissionSpec, ...] = ()) -> tuple[RawPermissionEntry, ...]:
        if not self.config_path.is_file():
            return ()
        document = self._load_document(self.config_path)
...
def build_permissions_adapters(
    kernel: HarnessKernelService,
) -> tuple[FileBackedPermissionsAdapter, ...]:
    return tuple(
        ...
        for binding in kernel.bindings_for_family("permissions")
...
__all__ = ["FileBackedPermissionsAdapter", "build_permissions_adapters"]
